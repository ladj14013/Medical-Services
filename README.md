...
2.  **قاعدة بيانات MySQL (اختياري):** لتشغيل التطبيق مع قاعدة بيانات حقيقية، ستحتاج إلى خادم MySQL قيد التشغيل.
...
2.  **تثبيت الحزم:** قم بتشغيل الأمر التالي لتثبيت جميع الحزم والمكتبات التي يعتمد عليها المشروع.
    ...
    npm install
    ```
3. **إعداد متغيرات البيئة (اختياري):** إذا كنت تريد الاتصال بقاعدة بيانات MySQL، قم بإنشاء نسخة من ملف `.env.example` وسمها `.env` ثم املأ تفاصيل الاتصال بقاعدة البيانات الخاصة بك.
