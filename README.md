...
#### الخطوة 3: استخدام الطرفية (Terminal) لرفع المشروع

1.  افتح الطرفية (Terminal أو Command Prompt أو PowerShell).
2.  **انتقل إلى مجلد المشروع الذي قمت بفك ضغطه في الخطوة 1**. يمكنك استخدام الأمر `cd` (Change Directory) للوصول إليه. مثال: `cd Downloads/Medical-Services`.
3.  بمجرد دخولك إلى المجلد الصحيح، قم بتنفيذ الأوامر التالية بالترتيب:

    ```bash
    # 1. تهيئة Git لإنشاء مستودع محلي جديد.
    # الأمر -b main يضمن أن اسم الفرع الرئيسي هو main.
    git init -b main

    # 2. إضافة جميع الملفات في مشروعك إلى منطقة التجهيز (staging area).
    git add .

    # 3. عمل أول "commit"، والذي يحفظ لقطة من حالة مشروعك الحالية.
    git commit -m "Initial commit of medical services app"

    # 4. ربط المستودع المحلي بالمستودع البعيد على GitHub.
    # **هام:** استبدل الرابط أدناه بالرابط الذي حصلت عليه من صفحة مستودعك على GitHub.
    git remote add origin https://github.com/your-username/your-repository-name.git

    # 5. رفع (push) الكود من فرع main المحلي إلى مستودع origin البعيد على GitHub.
